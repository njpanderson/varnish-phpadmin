...
### 2) Deciding on `admin` or `http` for the BAN-ing method
...
**Note:** Even when using the `admin` connection type, PURGE requests will still be made over HTTP and the stats will only be available from the files collected with `varnishstat-json.sh` (See more on this below).

### 3) Using `varnishstat-json.sh`
...
Once you have the `index.php` and `settings.php` in a publicly accessible location on your web server, and the `varnishstat-json.sh` file running on a 1 minute interval, everything should work as expected.
